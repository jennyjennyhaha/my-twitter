...
class TweetTests(TestCase):
    ...
    def test_cache_tweet_in_redis(self):
        tweet = self.create_tweet(self.jenny)
        ...
        self.assertEqual(tweet, cached_tweet)  # compare content, not address
...
class TweetServiceTests(TestCase):
    ...
    def test_get_user_tweets(self):
        tweet_ids = []
        for i in range(3):
            tweet = self.create_tweet(self.jenny, 'tweet {}'.format(i))
        ...
        # cache miss
        tweets = TweetService.get_cached_tweets(self.jenny.id)
        ...
        # cache updated
        new_tweet = self.create_tweet(self.jenny, 'new tweet')
        tweets = TweetService.get_cached_tweets(self.jenny.id)
        tweet_ids.insert(0, new_tweet.id)
        self.assertEqual([t.id for t in tweets], tweet_ids)

    def test_create_new_tweet_before_get_cached_tweets(self):
        tweet1 = self.create_tweet(self.jenny, 'tweet1')

        RedisClient.clear()
        conn = RedisClient.get_connection()

        key = USER_TWEETS_PATTERN.format(user_id=self.jenny.id)
        self.assertEqual(conn.exists(key), False)
        tweet2 = self.create_tweet(self.jenny, 'tweet2')
